...
class GetPrice {
    ...
    public function getForm(){
        ...
        $price = file_get_contents( DIR_PATH.'admin_panel/templates/price_one.tpl' );
        ...
        $html = file_get_contents( DIR_PATH.'admin_panel/templates/main_price.tpl' );
            ...
            for($j=0, $k=1; $j<1; $j++)
            {
                ...
                for (; $i<=$data->sheets[$j]["numRows"]; $i++)
                {
                    ...
                    if(!empty($data->sheets[$j]["cells"][$i][1])){
                        $sub_id = addslashes(trim($data->sheets[$j]["cells"][$i][1]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][2])){
                        $cell_id = addslashes(trim($data->sheets[$j]["cells"][$i][2]));
                    }
                    if($cell_id == '0')
                    {
                        continue;
                    ...
                    if(!empty($data->sheets[$j]["cells"][$i][4])){
                        $size  = addslashes(trim($data->sheets[$j]["cells"][$i][4]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][5])){
                        $p1    = addslashes(trim($data->sheets[$j]["cells"][$i][5]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][6])){
                        $p2    = addslashes(trim($data->sheets[$j]["cells"][$i][6]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][7])){
                        $p3    = addslashes(trim($data->sheets[$j]["cells"][$i][7]));
                    }


                    $query="INSERT INTO ".$tab." (`id`,`title`, `size`, `p1`, `p2`, `p3`, `cell_id`, `sub_id`) VALUES('".$k."', '".$title."', '".$size."', '".$p1."', '".$p2."', '".$p3."', '".$cell_id."', '".$sub_id."')";
                    $mysqli->query($query);
                    $k++;
                }
            }
        ...
        if(isset($_FILES["price1_upload"]) && !empty($_FILES["price1_upload"]["name"]) && (preg_match("/price_polochn/i", $_FILES["price1_upload"]["name"]))){
            ...
            for($j=0, $k=1; $j<1; $j++)
            {
                ...
                for (; $i<=$data->sheets[$j]["numRows"]; $i++)
                {
                    ...
                    if(!empty($data->sheets[$j]["cells"][$i][5])){
                        $power    = addslashes(trim($data->sheets[$j]["cells"][$i][5]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][6])){
                        $word    = addslashes(trim($data->sheets[$j]["cells"][$i][6]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][7])){
                        $p    = addslashes(trim($data->sheets[$j]["cells"][$i][7]));
                    }
                    if(!empty($data->sheets[$j]["cells"][$i][8])){
                        $weight    = addslashes(trim($data->sheets[$j]["cells"][$i][8]));
                    }


                    $query="INSERT INTO ".$tab." (`id`,`title`, `size`, `power`, `word`, `price`, `weight`, `cell_id`, `sub_id`) VALUES('".$k."', '".$title."', '".$size."', '".$power."', '".$word."', '".$p."', '".$weight."', '".$cell_id."', '".$sub_id."')";
                    $mysqli->query($query);
                    $k++;
                }
            }
    ...
    private function loadFile($tab, $file_array){
        ...
        $tmp_file_name = $file_array["tmp_name"];//echo DIR_PATH.PATH_EXCEL;
        $dest_file_name_loc = PATH_EXCEL_LOC.$file_name;
        $dest_file_name = DIR_PATH.PATH_EXCEL.$file_name;
