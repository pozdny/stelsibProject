...
function sendmail($arr, $str)
{
    ...
    require_once('../../'.M_SITE.'/lib/MailClass.inc');
...
function createMailMessage($arr, $str)
{
    if($str == 'predzakaz'){
        ...
        $html = file_get_contents('../../'.M_SITE.'/templates/tpl/predzakazMail.tpl');
        ...
        $phone = $arr["phone"];
        $caracter = $arr["caracter"];
        $html = file_get_contents('../../'.M_SITE.'/templates/tpl/backCallMail.tpl');
        $html = str_replace( '{name}',$name, $html);
        $html = str_replace( '{phone}',$phone, $html);
        $html = str_replace( '{comments}',$caracter, $html);
    }
