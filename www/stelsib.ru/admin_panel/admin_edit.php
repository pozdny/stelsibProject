...
require_once('../include/class/Autoloader.php');
    ...
    if (isset( $_POST["num"]))
    {
        ...
        switch($num)
        {
            ...
            case 4: edit_action($id, $value, $table);
                break;
            case 5: complects_action($id, $type, $table, $str);
                break;
...
function complects_action($id, $type, $table, $str){
    $mysqli = M_Core_DB::getInstance();
    ($type == 'false')? $p = 0:$p = 1;

        $query = sprintf("UPDATE ".$table." SET ".$str."=%s WHERE id=%s",
...
function put_action($id, $value, $table){
    ...
    elseif($table == TABLE_NEWS){
        ...
        foreach($value as $key=>$val){
            ...
            if($val['name'] == 'link')    $link    = ($val['value']);
            if($val['name'] == 'all')     $all     = ($val['value']);
        }
        $date = date('Y-m-d H:i:s');
        if($all){
            $reg_obj = new HostArr();
            $reg_arr = $reg_obj->getHostArr();
            foreach($reg_arr as $key => $value){
                if($value["eng"] !='')
                    $tab_reg = $table.'_'.$value["eng"];
                else{
                    $tab_reg = $table;
                }
                $query = sprintf('INSERT INTO '.$tab_reg.' (`title`, `content`, `date`) VALUES (%s, %s, %s)',
                    GetSQLValueString($title, 'text'),
